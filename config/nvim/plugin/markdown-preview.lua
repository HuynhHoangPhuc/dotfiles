require("packages.lazy").add({
	...
	ft = { "markdown" },
	config = function()
		-- Build the preview app's dependencies if they are missing. This used to
		-- run on every startup just to decide it had nothing to do; it now only
		-- costs anything the first time a markdown buffer is opened.
